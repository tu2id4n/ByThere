...
function PokerTreeBuilder:_get_children_nodes_chance_node(parent_node)
  ...
  -- guo change this to remove network
  --local next_boards = card_tools:get_boards(parent_node.street+1)
  local next_boards = card_tools:get_next_round_boards(parent_node.board)
  local next_boards_count = next_boards:size(1)
  -- guo add this
  --print("current board: ", parent_node.board,", next: ", next_boards_count, next_boards)
...
function PokerTreeBuilder:build_tree(params)
  ...
  -- guo add to visual tree
  local tree_visualiser = TreeVisualiser()
  tree_visualiser:graphviz(root, 'tree_no_limit_' .. os.time())
  return root
end
