...
local SubLookahead = torch.class('SubLookahead')
local timings = {}
--- Constructor
function SubLookahead:__init(terminal_equity, batch_size)
...
function SubLookahead:build_lookahead(tree)
  self.builder:build_from_tree(tree)
end

function SubLookahead:reset()
  self.builder:reset()
end
...
function SubLookahead:resolve_first_node(player_range, opponent_range)
...
-- before re-solving
function SubLookahead:resolve(player_range, opponent_cfvs)
  assert(player_range)
...
-- @local
function SubLookahead:_compute()
  --1.0 main loop
  ...
  for iter=1,arguments.sub_cfr_iters do
    timer:reset()
  ...
  timings[9] = timings[9] + timer:time().real
  timer:reset()
  self:_compute_normalize_average_cfvs()
  timings[10] = timings[10] + timer:time().real
  timer:reset()
  -- for i=1,10 do
  --   print(' ' .. i .. ': ' .. timings[i])
  -- end
...
-- @local
function SubLookahead:_compute_current_strategies()
...
-- @local
function SubLookahead:_compute_ranges()
...
-- @local
function SubLookahead:_compute_update_average_strategies(iter)
...
-- @local
function SubLookahead:_compute_terminal_equities_terminal_equity()
...
-- players' counterfactual values at the depth-limited states of the SubLookahead.
-- @local
function SubLookahead:_compute_terminal_equities_next_street_box()
...
function SubLookahead:get_chance_action_cfv(action, board)
...
-- @local
function SubLookahead:_compute_terminal_equities()
  --guo change this to remove last network
  --if self.tree.street ~= constants.streets_count then
...
-- @local
function SubLookahead:_compute_cfvs()
...
-- @param iter the current iteration number of re-solving
-- @local
function SubLookahead:_compute_cumulate_average_cfvs(iter)
  if iter > arguments.cfr_skip_iters then
...
-- @local
function SubLookahead:_compute_normalize_average_strategies()
...
-- @local
function SubLookahead:_compute_normalize_average_cfvs()
...
-- @local
function SubLookahead:_compute_regrets()
  for d=self.depth,2,-1 do
    local gp_layer_terminal_actions_count = self.terminal_actions_count[d-2]
...
function SubLookahead:get_results()
  local out = {}
...
-- @local
function SubLookahead:_set_opponent_starting_range(iteration)
  if self.reconstruction_opponent_cfvs then
